...
export class PlayerScene {
  ...
  _buildWorld(){
    ...
    this.player = Bodies.rectangle(W*0.25, H-100, this.pw, this.ph, {
      friction:0.001, frictionAir:0.02, restitution:0
  ...
  _drawGround(){
    const W = this.app.renderer.width, H = this.app.renderer.height;
    this.groundGfx.clear().beginFill(0x242423, 0.9).drawRect(0,0,W,36).endFill();
  ...
  _update = ()=>{
    ...
    // 착지 팝
    if (onGround && !this.wasOnGround) this.landPop = Math.min(Math.abs(this.player.velocity.y)/9, 0.5);
    ...
    let targetSX = 1 + (vy > 0 ? 0.22*t : -0.12*t) + this.landPop*0.6;
    let targetSY = 1 + (vy < 0 ? 0.35*t : -0.18*t) - this.landPop*0.5;
